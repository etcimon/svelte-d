...
`{#await job}` in svelte-d is still a `JsPromise` handle plus three `@visible` children. `App.ready` calls `wireAwait`. On a module the etcimon/binaryen fork asyncified, that is `job.await` then `libwasmAwaitFailed()`; stock Binaryen keeps the `.then` / `.error` pair so the page still settles. The promise must already be a `JsPromise`. A D `Promise!T` is the wrong type.
...
JS always **resolves** the import so rewind happens (otherwise `ScopedPool` dtors never run). A rejected Promise is stored as `libwasmAwaitFailed()` / `window.__svelteDLastAwait`. D reads that flag **after** `.await` returns, assigns `{:catch e}` from `libwasmAwaitError()`, and flips `{:catch}` visibility. A `try` that wraps the import is still unsound: rewind re-enters from the top and the personality slot is not part of Asyncify.
