...
Binaryen ≥123 parses this `try_table` module and is the official `-Oz` post-link. Do not compose it with Flatten `--asyncify`. Do not print `.await` on this cell. See [JsPromise](./async) and [wasm-eh](../advanced/wasm-eh).
