...
`svelte-d wasm` / `svelte-d build` are **release + `lflags -strip-all`**. `svelte-d wasm --debug` and `svelte-d host` keep symbols for IR work in the dest. On the kit-admin tree that is 12.64 MiB of debug wasm versus **1.59 MiB** stripped, and 14.08 MiB of debug host versus **10.85 MiB** release — see [Wasm and host sizes](./advanced/sizes).
