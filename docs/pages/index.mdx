...
The heart of the site is [Svelte → D IR](./language). That section teaches every interactive `.svelte` construct svelte-d prints, and the exact libwasm idiom it becomes. Read it before adding `{#each}{#if row.ok}` or `<svelte:boundary>` to a real page. [Reference](./reference) is the CLI and adapters. [Advanced](./advanced) covers the single LDC 1.43 cell, wasm exception handling, incremental compile, and the debug map.
