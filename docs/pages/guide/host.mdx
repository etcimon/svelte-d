...
The runtime those identifiers belong to — `listenHTTP`, `URLRouter`, `registerWebInterface`, `Json`, `connectDB` — is [vibe.0](../vibe.0). [lang="d" on the host cell](../vibe.0/lang-d) walks a full `+page.server.d` against that API. Open `svelte-engine-ws/webserver` in VS Code with code-d to complete the same names; see [Getting Started](../getting-started).
