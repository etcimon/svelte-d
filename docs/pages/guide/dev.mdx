...
If Vite is “file in use” on Windows after a crash, drop with `--force` rather than deleting the workspace by hand. `--force` keeps `node_modules`. Deleting the workspace by hand does not, and the next drop will reinstall it.
