...
What the script must not contain follows from the cell split. `import vibe.*` is the host cell. npm `import` is `lang=ts` plus Vite, or `Eval("window.$")`. `.await` is the asyncify cell, which this default wasm-eh module is not; use `JsPromise.then`. A function that both `try/catch`es and `.await`s is asking Flatten and `try_table` to share a frame, and Binaryen 132 will not do that.
