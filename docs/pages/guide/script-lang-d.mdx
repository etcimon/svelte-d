...
What the script must not contain follows from the cell split. `import vibe.*` is the host cell. npm `import` is `lang=ts` plus Vite, or `Eval("window.$")`. `.await` is printed in `wireAwait` when the etcimon Binaryen fork asyncified the wasm-eh module; wrap it in `try/catch` and you are asking Flatten and a landing pad to share a frame. Stock Binaryen 132 still cannot `--asyncify` `try_table`, so that path keeps `JsPromise.then`.
