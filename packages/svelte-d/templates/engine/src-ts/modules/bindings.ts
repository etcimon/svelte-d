...
export let jsExports = {
  env: {
    ...
    applyObjectSpread: (nodePtr: number, bagPtr: number) => {
      const el = libwasm.objects[nodePtr];
      const bag = libwasm.objects[bagPtr];
      if (!el || bag == null || typeof bag !== 'object') return;
      const keys = Object.keys(bag);
      for (let i = 0; i < keys.length; i++) {
        const k = keys[i];
        const v = (bag as Record<string, unknown>)[k];
        if (v === false || v == null) {
          if (typeof el.removeAttribute === 'function') el.removeAttribute(k);
          continue;
        }
