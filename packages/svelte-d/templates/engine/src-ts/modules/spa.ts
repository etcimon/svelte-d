  ...
  env: {
    ...
    setPropertyBool: (
      ...
      try {
        ...
        /* getter-only or wrong type: dataset, clientWidth, paused, files */
      }
    },
    setPropertyInt: (
      nodePtr: number,
    ...
    setProperty: (
      ...
      if (!node) return;
      try {
        if (node[prop] !== undefined) node[prop] = decoder.string(valueLen, valueOffset);
      } catch {
        /* getter-only or wrong type */
      }
    },
